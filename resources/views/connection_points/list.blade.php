@extends('layouts.app')
@section('title', 'Connection Points')
        ...
        <div class="col">
            <div class="page-title-box">
                ...
                <ol class="breadcrumb mb-0">
                    ...
                    <li class="breadcrumb-item active">Connection Points</li>
                </ol>
            </div>
        </div>
        <div class="col">
            <a href="{{ route('connection_points.create') }}">
                <button class="btn btn-success float-right">Add Connection Point</button>
            </a>
        </div>
    </div>
                        ...
                        <div class="table-responsive">
                            <table class="table table-striped">
                                ...
                                <tr v-for="item in items.data" :key="item.id">
                                    ...
                                    <td>
                                        <a :href="`/dashboard/customers?connectionPoint=${item.id}`" v-if="item.customer_count > 0">
                                            <button class="btn btn-primary btn-sm">[[ item.customer_count ]]</button>
                                        </a>
                                        <span v-else>[[ item.customer_count ]]</span>
                                    </td>
                                    <td>
                                        <a :href="`/dashboard/connection_points/${item.id}/edit`">
...
@section('scripts')
    ...
    <script>
        const url = '{{ route('connection_points') }}';
            ...
            methods: {
                getData(update = true) {
                    ...
                    axios.post(url, this.filters).then(response => {
                        ...
                        if (update) {
                            router.push({ path: 'connection_points', query: this.filters});
                        }
                ...
                deleteItem(id, name) {
                    Swal.fire({
                        title: 'Are you sure?',
                        icon: 'danger',
                        html:`<form method="post" action="/dashboard/connection_points/${id}/delete">
                            @csrf
                        @method('DELETE')
                        <p>Are you sure you want to delete connection point ${name}?</p>
